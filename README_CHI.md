...
- **主要逻辑**:
  ...
  - 识别请求主体类型（USER 客户、ACCOUNT 门店员工 或 CONSUMER 顾客）。
...
| `X-User-Type` | 主体类型：`USER`（订阅客户）/ `ACCOUNT`（门店员工）/ `CONSUMER`（顾客） | `USER` |
| `X-User-Role` | 权限角色：`USER`, `OWNER`, `MANAGER`, `STAFF` | `OWNER` |
| `X-Org-Id` | 当前操作的门店/组织 ID | `org-123` |
| `X-Org-Name` | 组织名称 | `Tymoe Main Store` |
| `X-Device-Id` | (可选) POS/Kiosk 设备 ID | `dev-789` |
| `X-All-Org-Ids`| USER 关联的所有组织 ID (逗号分隔) | `org-1,org-2` |

> 不再注入 `X-Product-Type`：`productType` 已从 token 顶层字段移除，改为挂在每个
> organization 记录内部（见 CLAUDE.md 2025-10-11 记录），下游服务需要时应从
> 组织信息里取，而不是依赖网关 header。
...
---
*文档版本: 1.1（2026-07-07，已同步移除 X-Product-Type 依赖）*
